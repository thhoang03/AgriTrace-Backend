...
* `Repository`: Thực hiện truy vấn thực tế, chịu trách nhiệm chuyển đổi hai chiều (manual mapping) giữa `DataModel` và `Domain Entity` (qua constructor/property của Entity, không dùng Mapster để bảo vệ tính đóng gói của Domain).
