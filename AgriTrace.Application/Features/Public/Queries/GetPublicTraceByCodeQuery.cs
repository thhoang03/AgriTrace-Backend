...
public record GetPublicTraceByCodeQuery(string BatchCode) : IRequest<PublicTraceDto>;

public class GetPublicTraceByCodeQueryHandler : IRequestHandler<GetPublicTraceByCodeQuery, PublicTraceDto>
{
    ...
    public GetPublicTraceByCodeQueryHandler(
    ...
    public async Task<PublicTraceDto> Handle(GetPublicTraceByCodeQuery request, CancellationToken cancellationToken)
    {
        var batch = await _batchReadService.GetByBatchCodeAsync(request.BatchCode, cancellationToken)
            ?? throw new NotFoundException($"Batch with code '{request.BatchCode}' not found.");
